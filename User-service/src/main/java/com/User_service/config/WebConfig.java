...
public class WebConfig implements WebMvcConfigurer {
    ...
    public void addResourceHandlers(ResourceHandlerRegistry registry) {
        String uploadsPath = Paths.get("User-service/uploads/").toAbsolutePath().toUri().toString();
        System.out.println("Serving files from: " + uploadsPath);

        // This URL pattern must match what the client requests
        registry.addResourceHandler("/api/users/files/**") // include /api if your gateway exposes /api
